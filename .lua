...
local function anyWorldEnabled() for ,v in pairs(worldEnabled) do if v then return true end end return false end
...
local col, tagName, textName = worldColors[cat], "VD"..cat, "VD_Text_"..cat
local n = 0
...
for ,entry in pairs(worldReg[cat]) do
if entry and entry.part then
clearChild(entry.part,"VD"..cat); clearChild(entry.part,"VD_Text_"..cat)
end
end
end
end
...
for ,part in ipairs(c:GetDescendants()) do
...
for part, in pairs(noclipTouched) do if part and part.Parent then part.CanCollide=true end end
noclipTouched={}
end
end
...
-- Fungsi untuk mencari pemain yang terhook
local function findPlayersOnHook()
    local hookedPlayers = {}
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LP and player.Team == LP.Team then
            ...
            if char and char.Parent and char:FindFirstChildOfClass("Humanoid") then
                -- Cek beberapa metode berbeda untuk mendeteksi hook
                local isHooked = false
                
                -- Metode 1: Cek atribut
                if char:GetAttribute("Hooked") == true or 
                   char:GetAttribute("IsHooked") == true then
                    isHooked = true
                end
                
                -- Metode 2: Cek posisi vertikal yang tidak normal
                local hrp = char:FindFirstChild("HumanoidRootPart")
                if hrp and hrp.Position.Y > 30 then -- Asumsi hooks biasanya tinggi
                    isHooked = true
                end
                
                -- Metode 3: Cek status humanoid
                local humanoid = char:FindFirstChildOfClass("Humanoid")
                if humanoid and humanoid.PlatformStand then
                    isHooked = true
                end
                
                -- Metode 4: Cek animasi yang mungkin dimainkan
                local animator = humanoid and humanoid:FindFirstChildOfClass("Animator")
                if animator then
                    for _, track in pairs(animator:GetPlayingAnimationTracks()) do
                        if track.Name:lower():find("hook") or 
                           track.Name:lower():find("hang") or
                           track.Name:lower():find("caught") then
                            isHooked = true
                            break
                        end
                    end
                ...
                -- Jika terhook, tambahkan ke daftar
                if isHooked then
                    table.insert(hookedPlayers, {player = player, char = char})
                end
            end
        end
    end
    
    return hookedPlayers
end

-- Fungsi untuk menemukan dan memicu rescue
local function tryRescuePlayer()
    if rescueDebounce then return end
    
    local hookedPlayers = findPlayersOnHook()
    if #hookedPlayers == 0 then return end
    
    for _, data in ipairs(hookedPlayers) do
        local player = data.player
        local char = data.char
        local hrp = char:FindFirstChild("HumanoidRootPart")
        
        if hrp then
            -- Cek jarak killer dari hook
            local killerDist = nearestKillerDistanceTo(hrp.Position)
            
            if killerDist > RESCUE_KILLER_SAFE_DIST then
                rescueDebounce = true
                
                -- Teleport ke lokasi rescue
                local rescuePos = hrp.Position + Vector3.new(0, 0, 3)
                tpCFrame(CFrame.new(rescuePos, hrp.Position))
                
                -- Tunggu sejenak
                task.wait(0.2)
                
                -- Coba temukan event rescue
                local found = false
                local remotes = ReplicatedStorage:FindFirstChild("Remotes")
                
                if remotes then
                    -- Coba beberapa kemungkinan nama event
                    local hookFolder = remotes:FindFirstChild("Hook") or remotes:FindFirstChild("Hooks")
                    
                    if hookFolder then
                        local rescueEvent = hookFolder:FindFirstChild("RescueEvent") or 
                                           hookFolder:FindFirstChild("Rescue") or
                                           hookFolder:FindFirstChild("UnhookPlayer")
                    ...
                    -- Coba juga di root Remotes
                    if not found then
                        local rescueEvent = remotes:FindFirstChild("RescueEvent") or 
                ...
                -- Coba interaksi langsung jika event tidak ditemukan
                if not found then
                    -- Cari proximity prompt yang mungkin digunakan untuk rescue
                    ...
                    -- Jika masih belum berhasil, klik beberapa kali di sekitar karakter
                    if not found then
                        for i = 1, 5 do
                            mouse1click()
                            task.wait(0.1)
                ...
                task.delay(4, function() rescueDebounce = false end)
                break -- Hanya rescue satu player dalam satu siklus
            end
        end
    end
end
        ...
        if state then
            ...
            rescueLoop = RunService.Heartbeat:Connect(function()
                if autoRescueEnabled and not rescueDebounce then
                    task.spawn(tryRescuePlayer) -- Run in separate thread to prevent hitches
                end
            end)
...
local STUN_OPTIMAL_DISTANCE = 7  -- Jarak optimal yang lebih besar untuk stun killer
local stunDebounce = false

-- Fungsi untuk mencari pallet terdekat yang belum dijatuhkan
local function findNearestPallet()
    local hrp = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then return nil, 999 end
    
    local closestPallet, closestDistance, closestEntry = nil, 15, nil
    
    -- Scan semua pallet yang terdaftar
    ...
    -- Scan juga pallet di map yang mungkin belum terdaftar
    local map = Workspace:FindFirstChild("Map")
    if map then
        for _, pallet in ipairs(map:GetDescendants()) do
            if (pallet.Name == "Pallet" or pallet.Name == "Palletwrong") and pallet:IsA("Model") then
                local part = firstBasePart(pallet)
                if part then
                    local distance = (part.Position - hrp.Position).Magnitude
                    if distance < closestDistance then
                        -- Cek jika pallet ini belum dijatuhkan
                        local dropped = pallet:GetAttribute("Dropped") or pallet:GetAttribute("Down") or false
                        local destroyed = pallet:GetAttribute("Destroyed") or false
                        
                        if not dropped and not destroyed then
                            closestPallet = pallet
                            closestDistance = distance
                            closestEntry = {part = part}
                        end
                    end
                end
            end
    ...
    return closestPallet, closestDistance, closestEntry
end

-- Fungsi untuk mendeteksi killer di dekat pallet
local function isKillerNearPallet(palletPart)
    if not palletPart then return false, 999 end
    
    local closestKiller, closestDist = nil, STUN_OPTIMAL_DISTANCE
        ...
        if player ~= LP and getRole(player) == "Killer" then
            ...
            if killerHRP then
                ...
                if dist < closestDist then
                    ...
                    -- Jika sudah sangat dekat, tidak perlu mencari lagi
                    if dist <= 3 then
                        return true, dist, player
                    end
                end
            end
        end
    ...
    return closestKiller ~= nil, closestDist, closestKiller
end
...
local function tryDropPallet()
    ...
    local palletModel, playerDistance, palletEntry = findNearestPallet()
    if not palletModel or not palletEntry or playerDistance > 10 then return end
    
    -- Periksa jika killer berada di dekat pallet
    local killerNear, killerDist = isKillerNearPallet(palletEntry.part)
    
    if killerNear and playerDistance < 10 then
        stunDebounce = true
        
        -- Temukan dan trigger event untuk drop pallet
        local dropEvent = nil
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        ...
        if remotes then
            ...
            if palletFolder then
                dropEvent = palletFolder:FindFirstChild("PalletDropEvent") or 
                           palletFolder:FindFirstChild("DropPallet") or
                           palletFolder:FindFirstChild("Drop")
            end
            
            -- Cek juga di root Remotes
            if not dropEvent then
                dropEvent = remotes:FindFirstChild("PalletDropEvent") or
                           remotes:FindFirstChild("DropPallet")
            end
        end
        
        -- Jika event ditemukan, fire dengan berbagai kemungkinan parameter
        if dropEvent and dropEvent:IsA("RemoteEvent") then
            -- Coba beberapa variasi parameter
            local success = false
            
            -- Versi 1: Dengan pallet model saja
            pcall(function()
            ...
            if not success then
                pcall(function()
                    dropEvent:FireServer(palletEntry.part)
                    success = true
                end)
            end
            ...
            Rayfield:Notify({
                Title = "Pallet Stun",
                Content = success and "Stunned killer! (" .. math.floor(killerDist) .. " studs)" or "Failed to drop pallet",
                Duration = 2
            })
        else
            Rayfield:Notify({
                Title = "Pallet Stun",
                Content = "Drop event not found",
                Duration = 2
            })
        end
...
-- Loop untuk pallet assist dengan error handling
...
TabPlayer:CreateToggle({
    ...
    Callback = function(state)
        ...
        if state then
            if palletAssistLoop then pcall(function() palletAssistLoop:Disconnect() end) end
            
            palletAssistLoop = RunService.Heartbeat:Connect(function()
                if palletAssistEnabled and not stunDebounce then
                    local success, err = pcall(tryDropPallet)
                    if not success and err then
                        -- Silent error handling
                        stunDebounce = true
                        task.delay(1, function() stunDebounce = false end)
                    end
                end
            end)
            ...
            if palletAssistLoop then 
                pcall(function() palletAssistLoop:Disconnect() end)
...
-- Variabel-variabel untuk ping display
local pingDisplayEnabled = false
local pingScreenGui = nil
local pingFrame = nil
local pingLabel = nil
local pingUpdateInterval = 1 -- Update lebih sering
local lastPingCheck = 0
local pingUpdateLoop = nil

-- Fungsi untuk membuat UI ping yang lebih stabil
local function createPingUI()
    -- Hapus UI lama jika ada
    if pingScreenGui then
        pcall(function() pingScreenGui:Destroy() end)
    end
    
    -- Buat ScreenGui baru
    pingScreenGui = Instance.new("ScreenGui")
    pingScreenGui.Name = "VD_PingDisplay"
    pingScreenGui.ResetOnSpawn = false
    pingScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    
    -- Buat frame untuk latar belakang
    ...
    -- Tambahkan corner untuk frame
    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0, 6)
    corner.Parent = pingFrame
    
    -- Tambahkan label untuk teks ping
    ...
    -- Parent ke PlayerGui
    pingScreenGui.Parent = LP.PlayerGui
    
    return true
end

-- Fungsi yang lebih handal untuk mendapatkan ping
local function getPing()
    ...
    -- Metode alternatif jika Stats API gagal
    if ping <= 0 then
        pcall(function()
            ping = math.floor(game:GetService("Stats"):GetTotalMemoryUsageMb())
        end)
    end
    
    -- Batasi nilai ping untuk menghindari anomali
    return math.clamp(ping, 0, 999)
end

-- Update warna label berdasarkan ping
local function updatePingDisplay()
    if not pingLabel then return false end
    
    local ping = getPing()
    
    -- Setel warna berdasarkan ping
    local textColor
    if ping < 100 then
        textColor = Color3.fromRGB(0, 255, 0) -- Green (good)
    elseif ping < 200 then
        textColor = Color3.fromRGB(255, 255, 0) -- Yellow (ok)
    else
        textColor = Color3.fromRGB(255, 0, 0) -- Red (bad)
    end
...
-- Toggle untuk ping display dengan pemulihan otomatis
TabMisc:CreateToggle({
    ...
    Callback = function(state)
        ...
        if state then
            -- Buat UI jika belum ada
            if not pingScreenGui or not pingScreenGui.Parent then
                if not createPingUI() then
                    Rayfield:Notify({
                        Title = "Error",
                        Content = "Failed to create ping display",
                        Duration = 3
                    })
                    return
                end
            ...
            -- Tampilkan UI
            pcall(function() pingScreenGui.Enabled = true end)
            
            -- Setup loop update
            if pingUpdateLoop then 
                pcall(function() pingUpdateLoop:Disconnect() end)
            end
            
            pingUpdateLoop = RunService.RenderStepped:Connect(function()
                ...
                if currentTime - lastPingCheck >= pingUpdateInterval then
                    ...
                    -- Check if UI still exists, recreate if needed
                    if not pingLabel or not pingLabel.Parent or not updatePingDisplay() then
                        createPingUI()
                        updatePingDisplay()
                    end
                end
            ...
            -- Sembunyikan UI tanpa menghancurkan
            pcall(function() 
                if pingScreenGui then 
                    pingScreenGui.Enabled = false
                end
            end)
...
-- Recreate UI jika PlayerGui berubah
LP:FindFirstChildOfClass("PlayerGui").ChildRemoved:Connect(function(child)
    if pingDisplayEnabled and child == pingScreenGui then
        task.wait(0.5)
